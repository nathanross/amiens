...
#                   ') ',
    ...
    def create_new(cls, l_db):
        ...
        (tmpId INTEGER PRIMARY KEY AUTOINCREMENT, ident TEXT UNIQUE, mediaType INTEGER NOT NULL DEFAULT 0, existsStatus INTEGER NOT NULL DEFAULT 0, totalAudioLength INTEGER, totalAudioSize INTEGER, checkedOnUnixDate INTEGER, hasMetadata INTEGER NOT NULL DEFAULT 0, metadata TEXT, blockDownload INTEGER NOT NULL DEFAULT 0, rating INTEGER NOT NULL DEFAULT 0, comment TEXT)''')
        conn.commit()
        conn.close()
        return cls(l_db)
